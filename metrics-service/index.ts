...
async function consumeMetrics() {
  ...
  console.log('🔍 Metrics Service: Testing database connection...');
  const maxRetries = 5;
  let retries = 0;
  let connected = false;
  ...
  if (!connected) {
    console.error('❌ Metrics Service: Failed to connect to PostgreSQL after multiple retries');
  ...
  console.log('✅ Metrics Service: Connected to PostgreSQL database');
